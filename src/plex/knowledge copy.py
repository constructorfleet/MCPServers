...
class PlexMediaPayload(BaseModel):
    ...
    def document(cls, item: "PlexMediaPayload") -> str:
        ...
        if item.rating:
            if item.rating > 90:
                parts.append("Rating: Excellent")
            elif item.rating > 75:
                parts.append("Rating: Great")
            elif item.rating > 50:
                parts.append("Rating: Good")
            elif item.rating > 25:
                parts.append("Rating: Okay")
            else:
                parts.append("Rating: Bad")
        return "\n".join(parts)
...
class PlexMediaQuery(PlexMediaPayload):
    ...
    type: Optional[Literal["movie"] |
                   Literal["episode"]] = None  # type: ignore
...
class Collection(CollectionInfo, Generic[TModel]):
    ...
    async def upsert_data(
        ...
        for item in data:
            ...
            vectors.append(
                Document(text=self.make_document(item),
                         model=self.model, options={"cuda": True})
            )
    ...
    async def search(
        self, data: PlexMediaPayload, limit: int | None = None
        ...
        query = Document(
            text=self.make_document(data), model=self.model, options={"cuda": True}
        )  # type: ignore
        _LOGGER.warn(
